...
export const syncRailgunTransactionsV2 = async (
  ...
  const chain = NETWORK_CONFIG[networkName].chain;
  console.log(`[SYNC-TXID] Calling engine.syncRailgunTransactionsV2 for ${networkName}...`);
  try {
    const result = await getEngine().syncRailgunTransactionsV2(chain, 'manual trigger');
    console.log(`[SYNC-TXID] ✅ Engine sync completed (result: ${result !== undefined ? 'defined' : 'undefined'})`);
    return result;
  } catch (error: any) {
    console.error(`[SYNC-TXID] ❌ Engine sync failed:`, error?.message);
    console.error(`[SYNC-TXID] Error stack:`, error?.stack);
    throw error;
  }
};
