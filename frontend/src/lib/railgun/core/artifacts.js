 ...
 * Artifact Management
 * 
 * Handles downloading and caching of ZKP artifacts.
...
} from '../../artifacts/artifact-util.js';
...
export const setArtifactStore = (store) => {
  artifactStore = store;
};

export const setUseNativeArtifacts = (useNative) => {
  useNativeArtifacts = useNative;
};

export const getArtifacts = async (inputs) => {
...
export const artifactGetterDownloadJustInTime = {
  assertArtifactExists,
  getArtifacts,
  getArtifactsPOI,
};

export const overrideArtifact = (artifactVariant, artifact) => {
