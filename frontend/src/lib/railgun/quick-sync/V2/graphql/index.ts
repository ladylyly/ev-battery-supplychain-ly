...
// CRITICAL: Patch @whatwg-node/fetch to use window.fetch so our override works
// This ensures GraphQL Mesh uses window.fetch instead of @whatwg-node/fetch
// This code executes immediately when the module is imported
console.log('[GraphQL Mesh] Module loading - checking for fetch override...');
let patchedFetchFn = fetchFn;
if (typeof window !== 'undefined' && window.fetch) {
  console.log('[GraphQL Mesh] Window.fetch available, creating patched version...');
  // Create a wrapper that uses window.fetch (which has our override)
  patchedFetchFn = function(...args: Parameters<typeof fetch>) {
    // Log all fetch calls for debugging
    const url = args[0]?.toString() || '';
    if (url.includes('rail-squid.squids.live/squid-railgun-eth-sepolia-v2')) {
      console.log('[GraphQL Mesh] 🚨 fetchFn (patched) called with default endpoint:', url);
      console.log('[GraphQL Mesh] 🚨 Redirecting to window.fetch which should intercept...');
    }
    // Use window.fetch to ensure our override intercepts
    return window.fetch(...args) as ReturnType<typeof fetch>;
  } as typeof fetch;
  console.log('[GraphQL Mesh] ✅ Patched fetchFn from @whatwg-node/fetch to use window.fetch');
  console.log('[GraphQL Mesh] 📍 patchedFetchFn will use window.fetch for all requests');
} else {
  console.warn('[GraphQL Mesh] ⚠️ Window.fetch not available, using default fetchFn');
}
...
// Log override check for debugging
...
// Dynamically get endpoint - check for override each time getMeshOptions is called
// Note: process.env.REACT_APP_* is replaced by webpack at build time
// Access directly without optional chaining on process.env
function getSepoliaEndpoint() {
  ...
  // Default to public Squids.live endpoint (fallback if no override is set)
  // To use local Subsquid indexer, set window.__OVERRIDE_SEPOLIA_V2_SUBGRAPH__ = 'http://localhost:4000/graphql'
  const endpoint = overrideURL || "https://rail-squid.squids.live/squid-railgun-eth-sepolia-v2/graphql";
  if (overrideURL) {
    console.log('[GraphQL Mesh] sepoliaHandler using override endpoint:', overrideURL);
  } else {
    console.log('[GraphQL Mesh] sepoliaHandler using default public endpoint:', endpoint);
  }
  return endpoint;
}

// Create handler with dynamic endpoint that checks override
// ALWAYS use window.fetch for Sepolia to ensure our override works
const sepoliaHandler = new GraphqlHandler({
              name: "sepolia",
              config: {
                "endpoint": getSepoliaEndpoint(),
                // CRITICAL: Use customFetch with window.fetch to bypass @whatwg-node/fetch
                // This ensures our window.fetch override can intercept requests
                "customFetch": typeof window !== 'undefined' && window.fetch ? (async (url: string | URL, init?: any) => {
                  // ALWAYS check for override at request time (not just at handler creation)
                  const overrideURL = (typeof process !== 'undefined' && process.env && process.env.REACT_APP_RAILGUN_SEPOLIA_V2_SUBGRAPH_URL) ||
                                      (typeof window !== 'undefined' && (window as any).__OVERRIDE_SEPOLIA_V2_SUBGRAPH__);
                  const urlString = url?.toString() || '';
                  
                  // Log ALL Sepolia requests for debugging
                  console.log('[GraphQL Handler] customFetch called:', urlString);
                  
                  // Intercept if it's the default endpoint and we have an override
                  if (overrideURL && (urlString.includes('rail-squid.squids.live/squid-railgun-eth-sepolia-v2') || urlString === getSepoliaEndpoint())) {
                    console.log('[GraphQL Handler] ✅ customFetch intercepting:', urlString, '→', overrideURL);
                    return window.fetch(overrideURL, init);
                  }
                  
                  // Always use window.fetch (not @whatwg-node/fetch) to ensure our override works
                  console.log('[GraphQL Handler] customFetch using window.fetch for:', urlString);
...
// If override is detected, replace sepolia handler with one that uses override endpoint
// This ensures the handler is created with the correct endpoint at runtime
const overrideURL = (typeof process !== 'undefined' && process.env && process.env.REACT_APP_RAILGUN_SEPOLIA_V2_SUBGRAPH_URL) ||
                    (typeof window !== 'undefined' && (window as any).__OVERRIDE_SEPOLIA_V2_SUBGRAPH__);
if (overrideURL) {
  console.log('[GraphQL Mesh] Replacing sepolia handler with override endpoint:', overrideURL);
  const newSepoliaHandler = new GraphqlHandler({
    name: "sepolia",
    config: {
      "endpoint": overrideURL,
      // CRITICAL: Also add customFetch to replacement handler to use window.fetch
      "customFetch": typeof window !== 'undefined' && window.fetch ? (async (url: string | URL, init?: any) => {
        const urlString = url?.toString() || '';
        console.log('[GraphQL Handler] customFetch (replacement) called:', urlString);
  ...
  sources[4] = {
    name: 'sepolia',
    handler: newSepoliaHandler,
    transforms: sepoliaTransforms
  };
  console.log('[GraphQL Mesh] ✅ Sepolia handler replaced with override endpoint:', overrideURL);
}
...
const merger = new(StitchingMerger as any)({
  ...
  return {
    ...
    fetchFn: patchedFetchFn, // Use patched version that uses window.fetch
  };
}
