...
export class WalletPOINodeInterface extends POINodeInterface {
  ...
  async getPOIsPerList(
    ...
    console.log(`[POI-INTERFACE] getPOIsPerList called by SDK (refreshReceivePOIsAllTXOs)`);
    console.log(`[POI-INTERFACE] This ONLY queries status - does NOT submit proofs`);
    console.log(`[POI-INTERFACE] Chain: ${chain.type}:${chain.id}, Commitments: ${blindedCommitmentDatas.length}, ListKeys: ${listKeys.length}`);
  ...
  async submitPOI(
    ...
    console.log(`[POI-INTERFACE] submitPOI called by SDK!`);
    console.log(`[POI-INTERFACE] This is for transaction proofs (not received UTXO proofs)`);
    console.log(`[POI-INTERFACE] Chain: ${chain.type}:${chain.id}, ListKey: ${listKey}`);
    console.log(`[POI-INTERFACE] TXID Index: ${txidMerklerootIndex}, Commitments: ${blindedCommitmentsOut.length}`);
